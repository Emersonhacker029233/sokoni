...
class SellerRegistrationTest extends TestCase
{
    ...
    /**
     * A6 (tester feedback): shop registration's category list was missing
     * whereNull('parent_id') — a pre-existing form that never got updated
     * once the subcategory taxonomy landed — so it offered all ~89
     * categories flat and mixed, while the product-posting form
     * (ShopProductsController) correctly shows only the 13 top-level ones.
     * A shop's own category has never been meant to be a subcategory pick.
     */
    public function test_the_registration_forms_category_list_matches_the_product_forms_top_level_only_rule(): void
    {
        (new \Database\Seeders\CategorySeeder)->run();
        $electronics = \App\Models\Category::whereNull('parent_id')->where('name_en', 'Electronics')->firstOrFail();
        $tvs = \App\Models\Category::where('parent_id', $electronics->id)->where('name_en', 'TVs')->firstOrFail();
    ...
    public function test_submitting_valid_details_creates_a_pending_seller_profile(): void
    {
        Storage::fake('public');
        ...
        $this->assertSame(str_repeat('1', 20), $seller->nida_number);
    }
    ...
    public function test_registration_requires_the_nida_number(): void
    {
        Storage::fake('public');
        $user = $this->onboardedBuyer();
        $category = Category::factory()->create();
        $payload = $this->validPayload($category);
        unset($payload['nida_number']);

        $response = $this->actingAsWebUser($user)->post(route('web.account.shop.register.store'), $payload);

        $response->assertSessionHasErrors(['nida_number']);
