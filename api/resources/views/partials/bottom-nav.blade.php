            ...
            @switch($item['route'])
                ...
                @case('web.chats')
                    ...
                    @if (($unreadMessagesCount ?? 0) > 0)
                        <span class="absolute right-[22%] top-2 flex h-14 min-w-14 items-center justify-center rounded-full bg-sokoni-danger px-2 text-[10px] font-semibold text-white">{{ $unreadMessagesCount > 9 ? '9+' : $unreadMessagesCount }}</span>
                    @endif
                    @break
                @default
