...
<a href="{{ $productUrl }}" class="card group flex flex-col overflow-hidden transition hover:shadow-md">
    <div class="relative aspect-square overflow-hidden bg-sokoni-surface-alt">
        @if ($cover)
            <img
    ...
    <div class="flex flex-1 flex-col gap-4 p-12">
        <div class="flex items-baseline gap-8">
            <span class="text-base font-bold">{{ \App\Support\Money::format($offer ? $offer->discountedPrice() : $product->price) }}</span>
            @if ($offer)
                <span class="text-xs text-sokoni-black/40 line-through">{{ \App\Support\Money::format((int) $offer->price_snapshot) }}</span>
            @endif
        </div>

        <p class="line-clamp-2 text-sm text-sokoni-black/80">{{ $product->title }}</p>

        <div class="mt-auto flex items-center gap-4 pt-4 text-xs text-sokoni-black/50">
