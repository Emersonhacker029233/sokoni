...
<div class="mx-auto max-w-md px-16 py-48">
    <h1 class="text-xl font-bold">Before you continue</h1>
    <p class="mt-8 text-sm text-sokoni-black/60">Please review and accept our Terms and Privacy Policy to continue.</p>
    ...
    <form action="{{ route('web.auth.terms.store') }}" method="post" class="mt-24">
        ...
        <label class="flex items-start gap-8 text-sm">
            ...
            I have read and agree to the Terms of Service and Privacy Policy (version {{ \App\Support\Legal::TERMS_VERSION }}).
        </label>
        @error('accepted')
            <p class="mt-8 text-sm text-sokoni-danger">{{ $message }}</p>
        @enderror
        <button type="submit" class="btn-primary mt-20 w-full py-12">Continue</button>
    </form>
</div>
@endsection
